...
jobs:
  guardana:
    ...
    steps:
      ...
      - uses: guardana/guardana@v0.24   # moving tag; pins to the latest 0.24.x
        with:
          path: .
...
```bash
docker run --rm -v "$PWD:/work:ro" ghcr.io/guardana/guardana:0.24 \
...
```yaml
include:
  - remote: "https://raw.githubusercontent.com/guardana/guardana/v0.24/deploy/ci/gitlab-ci.yml"
